...
# Simulate a list of registered users (username and password)
user_credentials = {
    "admin": "admin123",  # Example admin user
    "john_doe": "password123"  # Example donor user
}
...
def register_donor():
    ...
    # Check if user entered "unknown" for HIV and Blood Type
    if hiv_status not in ["yes", "no", "unknown"]:
        print("Invalid HIV status input. Please type 'yes', 'no', or 'unknown'.")
        hiv_status = "unknown"  # Default to unknown if invalid input is given

    if blood_type not in ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-", "unknown"]:
        print("Invalid blood type input. Please use a valid blood type, or 'unknown' if you don't know.")
        blood_type = "unknown"  # Default to unknown if invalid input is given
