...
def collect_application_details():
    ...
    while True:
        ...
        if position_choice in ['1', '2', '3', '4']:
            ...
            print("Invalid choice! Please select a valid position number (1, 2, 3, or 4).")
    ...
    # Collecting address (new addition)
    address = input("Address: ")

    # Collecting social media link (LinkedIn or other)
    social_media = input("Social Media Link (optional - e.g., LinkedIn): ")
    ...
    # Collecting GitHub and LeetCode details if applying for Software Developer
    github_link = ""
    leetcode_link = ""
    if position == "Software Developer":
        github_link = input("GitHub Profile Link (optional): ")
        
        # Ask if they want to use the same account for LeetCode
        same_account_choice = input("Do you want to use the same GitHub link for your LeetCode profile? (Yes/No): ").strip().lower()
        if same_account_choice == 'yes':
            leetcode_link = github_link  # Using GitHub link as LeetCode link
        else:
            leetcode_link = input("LeetCode Profile Link (optional): ")

        # Collecting portfolio website link for Software Developer
        portfolio_link = input("Portfolio Website Link (optional): ")
    else:
        portfolio_link = ""

    # Collecting CV file path (new addition)
    cv_path = input("CV File Path (optional - provide the file location): ")

    # Collecting the details in a list
    applicant = [name, age, cgpa, position, email, phone_number, address, social_media, work_experience, skills, github_link, leetcode_link, portfolio_link, cv_path]
...
def save_to_files(applications):
    positions = {
        ...
        "Sales": "Sales.csv",
        "Marketing Manager": "Marketing_Manager.csv"
    }
    ...
    for position, filename in positions.items():
        with open(filename, mode='a', newline='') as file:
            ...
            if os.stat(filename).st_size == 0:  # Write header if file is empty
                writer.writerow(["Name", "Age", "CGPA", "Position", "Email", "Phone Number", "Address", "Social Media", "Work Experience", "Skills", "GitHub Link", "LeetCode Link", "Portfolio Link", "CV File Path"])
...
def filter_best_applicant(position):
    ...
    try:
        ...
        with open(filename, mode='r') as file:
            ...
            applicants = []
            for row in data[1:]:
                name, age, cgpa, pos, email, phone, address, social_media, work_exp, skills, github, leetcode, portfolio, cv = row
                applicants.append({
                    "full-Name": name,
                    "Age": age,
                    "CGPA": float(cgpa),
    ...
    while True:
        ...
        if hr_choice == '1':
            ...
            if position_choice == '1':
                ...
                filter_best_applicant("Sales")
            elif position_choice == '4':
                filter_best_applicant("Marketing Manager")
            else:
