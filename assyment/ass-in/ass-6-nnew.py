...
def collect_application_details():
    ...
    # Collecting GitHub link if applying for Software Developer (new addition)
    github_link = ""
    ...
    applicant = [name, age, cgpa, position, email, phone_number, address, social_media, work_experience, skills, github_link, portfolio_link, cv_path]
    ...
    for position, filename in positions.items():
        with open(filename, mode='a', newline='') as file:
            ...
            if os.stat(filename).st_size == 0:  # Write header if file is empty
                writer.writerow(["Name", "Age", "CGPA", "Position", "Email", "Phone Number", "Address", "Social Media", "Work Experience", "Skills", "GitHub Link", "Portfolio Link", "CV File Path"])
...
def filter_best_applicant(position):
    ...
    try:
        ...
        with open(filename, mode='r') as file:
            ...
            applicants = []
            for row in data[1:]:
                name, age, cgpa, pos, email, phone, address, social_media, work_exp, skills, github, portfolio, cv = row
                applicants.append({
                    "full-Name": name,
                    "Age": age,
                    "CGPA": float(cgpa),
